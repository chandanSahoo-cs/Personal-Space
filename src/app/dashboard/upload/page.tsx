...
const getFileIcon = (mimeType: string) => {
  ...
  if (mimeType.startsWith("video/")) return <FileVideoIcon className="w-6 h-6 text-purple-400" />
  if (mimeType.startsWith("audio/")) return <FileAudioIcon className="w-6 h-6 text-orange-400" />
  if (mimeType.includes("pdf")) return <FileTextIcon className="w-6 h-6 text-red-400" />
...
const FilePage = () => {
  ...
  const [DeleteDialog, confirmDelete] = useConfirm("Delete File", "Are you sure you want to delete file?")
  ...
  return isOpen ? (
    ...
    <>
      ...
      <UploadDialog isOpen={dialogOpen} onClose={dialogClose} onUpload={handleUpload} uploading={uploading} />
      <div className="min-h-screen bg-[url('/image1.jpg')] bg-cover bg-center bg-no-repeat p-8 flex">
        <div className="p-8 space-y-8 bg-[#1d1f21]/90 backdrop-blur-sm rounded-3xl shadow-xl border border-[#2e3136] w-[900px] mx-auto  text-gray-100">
          ...
          {isMultiSelectMode && (
            <div className="flex items-center justify-between gap-4 p-4 bg-[#2e3136]/50 rounded-xl border border-[#40444b]">
              <div className="flex items-center gap-4">
                ...
                >
                  {selectedFiles.size === parsedFiles.length ? "Deselect All" : "Select All"}
                </Button>
          ...
          <div className="space-y-4">
            <h2 className="text-xl font-semibold text-gray-50">Your Uploaded Files</h2>
            {files.length === 0 ? (
              ...
              <div className={cn("grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1 gap-y-0 overflow-y-auto h-[410px] contest-scrollbar",
                ...
                {parsedFiles.map((file) => (
                  <Card
                    ...
                    key={file.id}
                    className={cn("bg-[#1d1f21]/80 backdrop-blur-sm border rounded-xl h-44 shadow-lg hover:shadow-xl hover:bg-[#1d1f21]/90 hover:border-[#5865f2]/40 transition-all duration-300 mr-2 group",
                      isMultiSelectMode && "h-28",
                      isMultiSelectMode && selectedFiles.has(file.id) ? "border-[#5865f2] bg-[#5865f2]/10" : "border-[#2e3136]/60"
                    )}
                  >
                    <CardContent className="pl-4 pr-4 pt-4 flex flex-col h-full justify-between">
                      ...
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div className="flex-grow min-w-0">
                          <p className="font-semibold text-gray-50 truncate text-sm leading-tight">{file.name}</p>
                          <div className="text-xs text-gray-400 mt-1 flex justify-between items-center">
                            <span className="capitalize truncate leading-tight w-[100px]">{file.type.split("/")[1] || file.type}</span>
